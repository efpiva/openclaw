...
function cloneSubagentRunMap(runs: Map<string, SubagentRunRecord>): Map<string, SubagentRunRecord> {
  return profileHotpathSync(
    "subagent.registry.cloneMap",
    { runCount: runs.size },
    () => new Map([...runs].map(([runId, entry]) => [runId, cloneSubagentRunRecord(entry)])),
  );
}
...
export function loadSubagentRegistryFromDisk(): Map<string, SubagentRunRecord> {
  ...
  return profileHotpathSync("subagent.registry.loadDisk", profileFields, () => {
    ...
    profileFields.hasFile = true;
    const cached = registryReadCache.get(pathname);
    if (cached?.signature === signature) {
      registryReadCache.delete(pathname);
      registryReadCache.set(pathname, cached);
      profileFields.cacheHit = true;
      profileFields.runCount = cached.runs.size;
    ...
    profileFields.runCount = out.size;
    if (migrated) {
      profileFields.migrated = true;
      try {
