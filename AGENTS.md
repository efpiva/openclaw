...
- If approval is required, run `scripts/codeclaw_approval_gate.py request` to create the pending approval token and render the approval request, including the paused PR `session_key`, only after checking for an existing approval artifact for the exact repo/PR/head/base/local-commit tuple. Approval-token creation is an external state change: do not create a second token for the same tuple because a cron reran, a prior token expired, or message delivery failed. Reuse the existing canonical token in the DM/session blocker, and if duplicate artifacts already exist, pick one canonical token, record the duplicate in memory if useful, and stop creating more until the head/base/local commit or explicit human instruction changes. The canonical approval artifact must live in the workspace/global approval store (for example `.codeclaw/approvals/` under this CodeClaw workspace), not only inside a per-PR scratch worktree that cleanup may delete; before removing a worktree, verify any newly-created approval artifact is present in the canonical store. If a later run finds the artifact missing but memory/session receipts show a token and successful Eduardo DM for the same repo/PR/head/base/local-commit, reconstruct that same canonical artifact if needed but do **not** send another approval DM and do **not** mint a replacement token. If the only change while a local commit is waiting for approval is that the base branch moved, perform the minimum freshness/merge-tree check; when it is still clean, do **not** refresh the approval token or send a new approval DM solely for the new base. Treat base-only movement as **not** a token-regeneration trigger even though it changes the exact approval tuple and would make `assert-approved` fail later; the right state is "stale but still waiting," not a renewed token. Record that the old token is stale in memory and wait for a fresh source/feedback/head trigger or explicit human approval/deny/instruction before regenerating approval state.
