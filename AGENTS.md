...
⚠️ CRITICAL — TELEGRAM NOTIFICATION: You are running as a subagent. The auto-announce mechanism DOES NOT WORK. For write-mode workflows, call the `message` tool at the end only when the run changed external state, found/reporting a blocker, resolved a conflict, pushed a commit, posted/replied on GitHub, or needs human attention. If a scheduler/triage tick prepares a dispatch plan and finds 0 actions, exit silently: no DM, no group/topic message, no "tick complete" summary, and no memory note unless this is the first observation of a new noise regression. If the mandatory duplicate-action guard determines the tick is already handled / approval-only / unchanged pending-watch, exit quietly: no PR comment, no Telegram digest, no "still clean" update, and no explanatory final assistant summary. Log at most one compact memory note per PR/head/state/day for unchanged duplicate/pending-watch ticks; if the same approval token, feedback/thread set, head, base-only movement status, and gate classification were already recorded that day, do not append another daily-log entry. For duplicate pending-approval/no-op reruns, treat CI rerun ids, build ids, token expiry age, and pass/fail detail changes inside the same already-classified non-actionable gate class as the same state; once today's note says the patch still applies and no source-actionable gate/comment changed, later reruns must skip memory entirely unless the PR head, unresolved feedback set, approval token, merge/conflict result, source-actionable gate class, delivery state, or explicit human instruction changes. For duplicate/no-op exits from Telegram-delivered sessions, make the final assistant text exactly `ANNOUNCE_SKIP` so routing suppresses visible no-op chatter.
...
- **Issue-PR self-review exception:** if `own_pr_self_review` is running on a PR created by CodeClaw from a `codeclaw`-labeled issue (for example the PR closes a `codeclaw` issue and its branch starts with `codeclaw/issue-`), CodeClaw is still pre-authorized to push self-review/reviewer-fix commits and mark the draft PR ready after self-review passes. Do not create `push-pr-*` approval tokens for this path. Treat the self-review fix as part of the original issue-fix PR lifecycle; CI gates and human PR review provide the oversight.
- **Default to approval-required for other existing PRs.** Human approval is required before pushing any CodeClaw-authored change to an existing PR (`own_pr_self_review` / `own_pr_comment_response`) unless the PR matches the Issue-PR self-review exception or the approval gate and the diff prove it is purely non-behavioral. Behavior-changing includes runtime, deployment, release, pipeline, security, data, routing, auth, scheduling, or production behavior; source-code behavior changes; CI/release YAML changes; generated deployment artifact changes; config/default changes; scripts; migrations; and docs/runbook changes that instruct operators to do something behavior-changing.
- If approval is required, run `scripts/codeclaw_approval_gate.py request` to create the pending approval token and render the approval request, including the paused PR `session_key`, only after checking for an existing approval artifact for the exact repo/PR/head/base/local-commit tuple. Approval-token creation is an external state change: do not create a second token for the same tuple because a cron reran, a prior token expired, or message delivery failed. Reuse the existing canonical token in the DM/session blocker, and if duplicate artifacts already exist, pick one canonical token, record the duplicate in memory if useful, and stop creating more until the head/base/local commit or explicit human instruction changes. The canonical approval artifact must live in the workspace/global approval store (for example `.codeclaw/approvals/` under this CodeClaw workspace), not only inside a per-PR scratch worktree that cleanup may delete; before removing a worktree, verify any newly-created approval artifact is present in the canonical store. Pending approval commits must also be durable outside the scratch worktree: preserve a shared-cache ref or bundle plus a canonical patch under this workspace (for example `.codeclaw/pending-patches/`) and verify the referenced commit is reachable before cleanup. If a later run finds the artifact, ref, patch, or commit object missing but memory/session receipts show an already-DM'd token for the same repo/PR/head/local change, reconstruct the same pending state and canonical token from the durable patch/receipts when possible; do **not** supersede it, send another approval DM, or mint a replacement token solely because local scratch/cache state was lost. Before preparing a new approval-gated commit, also search canonical approval artifacts, pending-patch records, recent memory, and recent same-PR sessions for an already-waiting token on the same repo/PR/head and same unresolved feedback/thread set; if that pending commit already covers the feedback, reuse or reconstruct that existing pending state instead of creating a narrower, rebased, reformatted, or otherwise alternate commit/token. Make that search an explicit approval preflight: do it before conflict-resolution/candidate prep when feedback/head identity is known, and repeat it immediately before running `scripts/codeclaw_approval_gate.py request`; never run `request` speculatively to rediscover, re-render, or re-DM an existing approval state. For base-move or merge-conflict runs, this preflight happens before `git merge origin/<base>`, cherry-pick, rebase, or manual conflict resolution; if memory/session receipts show an existing waiting token for the same PR head and same conflict/feedback class, stop as stale/still-waiting even when the local worktree, canonical artifact, or old candidate commit is missing. If a duplicate candidate or request artifact is discovered after late detection, mark it superseded/private, do not deliver it, and treat the missed preflight as a noise regression to record once. Before running `scripts/codeclaw_approval_gate.py request`, confirm the current subagent exposes the native `message` tool needed to DM Eduardo and consult today's memory/recent same-runtime delivery receipts for any native Telegram `message(action="send")` failure, including failures to #general/PR-topic as well as direct DMs. Treat tool-layer failures such as `Poll fields require action "poll"` as a runtime-wide approval-DM outage until a later direct Eduardo DM succeeds in the same runtime/date; do not assume a DM target will work just because the failed receipt was for a group/topic target. If the tool is unavailable or the current runtime/date already has that known delivery failure, stop before token creation, preserve at most the candidate patch/ref/bundle, and record a bounded blocker (`approval DM unavailable; no push/PR comment`) without a `push-pr-*` token. Do not create an approval artifact merely to stage work for a later run that might have delivery. At the start of every own-PR write-mode run, perform and record a delivery-outage preflight before preparing or classifying any behavior-changing candidate: check today's memory and recent same-runtime receipts for native Telegram `message(action="send")` failures or missing `message` tool availability. This preflight must include a deterministic scan of the current-day memory file for message-tool outage phrases such as `Poll fields require action "poll"`, `native message tool unavailable`, or `message tool unavailable`; if any such receipt exists and there is no later same-day successful direct Eduardo DM receipt, set the run state to `approval_dm_unavailable`. The scan must be a workspace-global current-day memory grep, not a same-PR/session-only lookup, and it must run before `scripts/codeclaw_approval_gate.py request` even when the candidate was prepared after a clean earlier preflight; if the current-day memory contains any approval-token-created/DM-failed receipt for another PR after an outage, treat that as proof the guardrail was missed and stop before minting another token. Repeat the same scan immediately before `scripts/codeclaw_approval_gate.py request`; never rely on a narrower same-PR/session search, successful PR-topic assistant text, or a successful non-Telegram side effect to clear a same-day native Telegram outage. Treat this repeat scan as a fail-closed shell gate, not advisory narration: capture the current-day grep result (or `no-outage-receipt`) in private run notes immediately adjacent to the would-be request, and if the scan finds `Poll fields require action`, `message tool unavailable`, any native Telegram send failure, or any current-day approval-token-created/DM-failed receipt with no later successful direct Eduardo DM, `scripts/codeclaw_approval_gate.py request` is forbidden. Preserving a patch/ref/bundle is the only allowed state change while that gate is closed, and a run that mints a `push-pr-*` token after such a receipt is itself a noise regression. If that preflight finds an outage, treat it as a hard guardrail: after any candidate is preserved, the run must stop before `request` even if validation/classification has completed. A later duplicate tick must not run `request` for the same candidate until a direct Eduardo DM has succeeded in this runtime/date. If delivery becomes available later and the same candidate still applies, create and DM the first token then. If the PR head moved but the same unresolved feedback/thread set is still covered by an existing waiting patch/commit, first test whether that patch applies cleanly to the new head and whether the head changed the touched files/behavior; when it still applies and the required fix is unchanged, reuse the existing canonical token/DM state instead of creating another token or DM. Create a replacement token for head movement only when the old patch no longer applies, conflict resolution or changed touched files materially alters the candidate diff, a fresh human/reviewer blocker expands the required fix, or Eduardo explicitly asks for regeneration. If the only change while a local commit is waiting for approval is that the base branch moved, perform the minimum freshness/merge-tree check; when it is still clean, do **not** refresh the approval token or send a new approval DM solely for the new base. Treat base-only movement as **not** a token-regeneration trigger even though it changes the exact approval tuple and would make `assert-approved` fail later; the right state is "stale but still waiting," not a renewed token. This also applies when the moved base makes the waiting patch/merge candidate conflict, stop applying directly, or require a materially different conflict resolution: classify it as "stale/conflicting but still waiting," do not proactively synthesize a new merge commit, create a replacement token, or send another approval DM. If Eduardo later approves the stale token and the exact candidate cannot be asserted/pushed against the current base, resolve/revalidate then and request a fresh approval only for the materially changed post-approval candidate. Do not append a new daily-log entry for every refreshed base SHA while the same approval token, PR head, feedback/thread set, merge-tree result or stale/conflicting status, gate classification, and waiting state are already recorded for the day; the base SHA changing again is not a new state by itself. Record that the old token is stale or stale/conflicting in memory only on the first observation or when the head, feedback set, merge result class, gate classification, delivery state, or explicit human instruction changes, then wait for a fresh source/feedback/head trigger or explicit human approval/deny/instruction before regenerating approval state.
- Send Eduardo a concise Telegram DM (`telegram` target `7570099326`, this direct conversation/session), not a PR/issue comment, review, group/topic announcement, or PR-session final text, with repo/PR, session key, head/base, commit SHA, files changed, behavior impact, reviewer/gate receipts, validation, and inline approval/deny controls when available. **Never post pending approval tokens, approval-command text, preserved candidate commit SHAs/patch refs, or “paused/waiting for approval” text to the PR/issue itself; it pollutes the review thread.** After the approval DM succeeds from a Telegram-delivered own-PR subagent, make the final assistant text exactly `ANNOUNCE_SKIP`; the DM is the human-visible approval summary, and repeating the token, commit SHA, patch/ref paths, or validation digest in the group/topic final message is duplicate approval noise. Treat `scripts/codeclaw_approval_gate.py request` output as sensitive delivery material: capture it to a private file/variable, DM it immediately with the `message` tool, and verify a successful `messageId` before any human-visible final text. If native DM delivery fails, is unavailable, or was skipped after an approval token/state is created, surface only a bounded delivery blocker in the current session and keep the PR run paused: do not include the approval token, approval-command text, approve/deny commands, patch path, commit details, files-changed list, validation checklist, or full validation digest in Telegram group/topic final assistant text. The bounded text may name only repo/PR, workflow, and "approval DM unavailable; no push/PR comment". A failed/skipped DM is not permission to create a PR comment/marker, issue comment, thread reply, group approval summary, “completed pending approval” final summary, or repeated token renewal. Do not post a PR comment merely to say a candidate was prepared, preserved, reused, or is waiting on approval; keep that state in private memory/pending-patch records until the approval DM succeeds or a push lands. The next duplicate tick must treat the unchanged approval state as already handled unless a fresh actionable trigger appears.
...
memory note if useful, then exit. A new human-authored head that self-review finds
clean is still not a reason for a marker-only/pass/no-code PR comment when no
commit was pushed, no blocker/conflict is being reported, no draft-ready state
changed, and gates are only pending or infra/non-actionable; memory is enough. Do
not replace the prohibited PR/Telegram summary with a verbose final assistant
recap; in Telegram-delivered subagent sessions, the final assistant message
itself can be routed to humans. After the memory note, return only the
...
7. Reply to individual threads/comments when possible, but avoid replying to
   ...
   commit was pushed, a blocker/conflict unrelated to approval delivery is being
   reported, or a human-requested answer cannot be delivered inline. If a fix was
   only prepared/preserved and approval DM delivery is unavailable, do not post a
   PR comment, marker, or thread reply that exposes the waiting approval state,
   token, candidate commit, patch/ref path, or validation digest; record the
   private pending state and exit bounded instead. If the filtered delta is
